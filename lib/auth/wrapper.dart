...
import 'package:studlife_chat/auth/register.dart';
...
class Wrapper extends StatelessWidget {
    ...
    return BlocProvider(
      ...
      child: BlocBuilder<AuthBloc, AuthState>(
        builder: (ctx, state) {
          ...
          {
            return _tryAgain(onPressed: (){BlocProvider.of<AuthBloc>(ctx).add(NetworkError());});
          }
          ...
          {
            return RoomsPage(askUsername: true,);
          }
        },
      ),
    );
  ...
  _tryAgain({onPressed})
  {
    return _ScafIt(child: Column(mainAxisAlignment: MainAxisAlignment.center,children: [
      RawMaterialButton(onPressed: onPressed,child: Icon(Icons.refresh),shape: CircleBorder(),fillColor: Colors.blueAccent,),
      SizedBox(height: 10,),
  ...
  _ScafIt({child})
  {
    return Scaffold(body: Center(child:child),backgroundColor: ThemeConstants.BGCOLOR_DARK,);
  }
}
